...
DEMO_PICTURE_PATH = str(
    Path(__file__).resolve().parents[1] / "assets/workstation-picture.svg"
)
...
class WorkstationDemo(Applet[DemoState]):
            ...
            children=[
                            ...
                            Grid(
                                ...
                                children=[
                                    GridChild(
                                        ...
                                        0,
                                        MetricCard(
                                            "CPU", state.cpu, "processor-symbolic"
                                        ),
                                    ),
                                    GridChild(
                                        0,
                                    ...
                                    GridChild(
                                        ...
                                        Progress(
                                            ...
                                            max=1,
                                            show_text=True,
                                            text=f"{int(state.disk * 100)}% disk",
                                        ),
                                    ),
                                    GridChild(
                                        1,
                ...
                Section(
                    ...
                    child=Column(
                        children=[
                            Switch(
                                id="vpn-toggle", label="VPN tunnel", active=state.vpn
                            ),
                            ToggleButton(
                                id="focus-toggle",
                                label="Focus mode",
                        ...
                        children=[
                            ...
                            Expander(
                                ...
                                child=Column(
                                    ...
                                    children=[
                                        Label(
                                            text=f"Profile: {PROFILES[state.profile]}"
                                        ),
                        ...
                        children=[
                            Row(
                                ...
                                children=[
                                    ...
                                    Label(
                                        text="Filter is handled through input callbacks; send event id=filter type=input."
                                    ),
                                ],
                            ),
                            Overlay(
                                child=Picture(
                                    path=DEMO_PICTURE_PATH, content_fit="cover"
                                ),
    ...
    async def change_brightness(self, event) -> None:
        await self.set_state(
            brightness=float(event.value), last_event="brightness changed"
        )
    ...
    async def change_profile(self, event) -> None:
        ...
        await self.set_state(
            profile=max(0, min(index, len(PROFILES) - 1)), last_event="profile changed"
        )
    ...
    async def scroll_pages(self, event) -> None:
        ...
        await self.set_state(
            page=((self.state.page + delta - 1) % 3) + 1, last_event="workspace changed"
        )
        ...
        child=Column(
            children=[
                Row(
                    spacing=6,
                    children=[
                        Icon(icon=icon_name, pixel_size=18),
                        Label(text=label),
                        Badge(label=f"{int(value * 100)}%"),
                    ],
                ),
                Progress(
                    value=value, max=1, show_text=True, text=f"{int(value * 100)}%"
                ),
            ]
        )
    )
