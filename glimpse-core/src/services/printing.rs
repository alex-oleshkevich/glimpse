...
fn cups_url() -> String {
    std::env::var("GLIMPSE_CUPS_URL").unwrap_or_else(|_| CUPS_URL.to_owned())
}
...
impl PrintingService {
    ...
    async fn run_inner(&mut self, cancel: CancellationToken) {
        tracing::debug!(url = %cups_url(), "printing service started");
    ...
    async fn poll(&mut self) {
        match fetch_state().await {
            ...
            Err(e) => {
                tracing::debug!(url = %cups_url(), error = %e, "printing: CUPS unavailable, will retry");
                let unavailable = State {
                    available: false,
                    ..State::default()
                };
                if *self.state_tx.borrow() != unavailable {
                    let _ = self.state_tx.send(unavailable);
                }
            }
        }
    }
...
async fn fetch_state() -> anyhow::Result<State> {
    ...
    let (printers, jobs) = tokio::try_join!(fetch_printers(uri.clone()), fetch_jobs(uri))?;
    Ok(State {
        available: true,
        printers,
        jobs,
...
async fn fetch_printers(uri: ipp::prelude::Uri) -> anyhow::Result<Vec<Printer>> {
    ...
    if resp.header().status_code().is_success()
        // client-error-not-found = no printers/jobs configured, treat as empty
...
async fn fetch_jobs(uri: ipp::prelude::Uri) -> anyhow::Result<Vec<PrintJob>> {
    use ipp::prelude::*;
...
async fn send_job_action(id: u32, action: JobAction) -> anyhow::Result<()> {
    ...
    let req: IppRequestResponse = match action {
        JobAction::Cancel => IppOperationBuilder::cancel_job(cups_uri.clone(), id as i32)
            .build()?
            .into_ipp_request(),
        JobAction::Pause => {
            ...
            req.attributes_mut().add(
                ...
                IppAttribute::with_name(IppAttribute::JOB_ID, IppValue::Integer(id as i32))?,
            );
            req
        }
        JobAction::Resume => {
...
fn parse_printers(groups: &IppAttributes) -> Vec<Printer> {
    groups
        ...
        .map(|group| {
            ...
            // IPP RFC 8011 §5.4.11: printer-state values 3=idle, 4=processing, 5=stopped
...
fn parse_jobs(groups: &IppAttributes) -> Vec<PrintJob> {
    groups
        ...
        .filter_map(|group| {
            ...
            // IPP RFC 8011 §5.3.7: job-state values 3-9
        ...
        // Stopped jobs require operator intervention; we hide them to keep the
        // applet focused on actionable jobs. They remain in parse_printers with
        // state_reasons so the error banner can surface them.
...
fn ipp_str_value(v: &ipp::value::IppValue) -> Option<String> {
    use ipp::value::IppValue;
    // Each variant wraps a different newtype, so we can't use | to unify them.
    match v {
        IppValue::TextWithoutLanguage(s) => Some(s.as_ref().to_owned()),
        IppValue::OctetString(s) => Some(s.as_ref().to_owned()),
...
fn str_attr(group: &IppAttributeGroup, name: &str) -> Option<String> {
    ipp_str_value(group.attributes().get(name)?.value())
}
...
fn i32_list_attr(group: &IppAttributeGroup, name: &str) -> Vec<i32> {
    use ipp::value::IppValue;
    let Some(attr) = group.attributes().get(name) else {
        return vec![];
    };
    match attr.value() {
        IppValue::Integer(n) => vec![*n],
        IppValue::Array(vals) => vals
            .iter()
            .filter_map(|v| {
                if let IppValue::Integer(n) = v {
                    Some(*n)
                } else {
                    None
                }
...
fn str_list_attr(group: &IppAttributeGroup, name: &str) -> Vec<String> {
    ...
    match attr.value() {
        IppValue::Array(vals) => vals.iter().filter_map(ipp_str_value).collect(),
