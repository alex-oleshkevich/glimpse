use std::{path::Path, sync::Arc};

use anyhow::{Context, anyhow};
use system_tray::{
    client::{ActivateRequest, Client, Event},
    item::{Category as SniCategory, IconPixmap, Status as SniStatus, StatusNotifierItem, Tooltip},
    menu::{
        Disposition, MenuItem as DbusMenuItem, MenuType, ToggleState, ToggleType,
        TrayMenu as DbusTrayMenu,
    },
};
use tokio::sync::{broadcast, mpsc};
use tokio_util::sync::CancellationToken;

use crate::dbus::status_notifier_item::StatusNotifierItemProxy;

use super::model::{
    Category, Icon, Item, MenuDisposition, MenuItem, MenuItemKind, MenuToggleState, MenuToggleType,
    Snapshot, Status,
};

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum TrayClientEvent {
    Changed { reason: String },
}

#[derive(Clone)]
pub struct TrayClient {
    client: Arc<Client>,
    session: zbus::Connection,
}

impl TrayClient {
    pub async fn new(session: zbus::Connection) -> anyhow::Result<Self> {
        Ok(Self {
            client: Arc::new(Client::new().await?),
            session,
        })
    }

    pub async fn snapshot(&self) -> anyhow::Result<Snapshot> {
        let entries = {
            let map = self.client.items();
            let guard = map
                .lock()
                .map_err(|_| anyhow!("tray client item map poisoned"))?;
            guard
                .iter()
                .map(|(address, (item, menu))| (address.clone(), (item.clone(), menu.clone())))
                .collect::<Vec<_>>()
        };

        Ok(snapshot_from_entries(&entries))
    }

    pub async fn listen(
        &self,
        events: mpsc::Sender<TrayClientEvent>,
        cancel: CancellationToken,
    ) -> anyhow::Result<()> {
        let mut rx = self.client.subscribe();

        loop {
            tokio::select! {
                _ = cancel.cancelled() => return Ok(()),
                event = rx.recv() => match event {
                    Ok(event) => {
                        if events.send(TrayClientEvent::Changed {
                            reason: event_reason(&event),
                        }).await.is_err() {
                            return Ok(());
                        }
                    }
                    Err(broadcast::error::RecvError::Lagged(skipped)) => {
                        tracing::warn!(skipped, "tray client event stream lagged");
                        if events.send(TrayClientEvent::Changed { reason: "lagged".into() }).await.is_err() {
                            return Ok(());
                        }
                    }
                    Err(broadcast::error::RecvError::Closed) => return Ok(()),
                }
            }
        }
    }

    pub async fn activate(&self, address: String, x: i32, y: i32) -> anyhow::Result<()> {
        self.client
            .activate(ActivateRequest::Default { address, x, y })
            .await
            .map_err(Into::into)
    }

    pub async fn secondary_activate(&self, address: &str, x: i32, y: i32) -> anyhow::Result<()> {
        self.item_proxy(address)
            .await?
            .secondary_activate(x, y)
            .await
            .map_err(Into::into)
    }

    pub async fn open_context_menu(&self, address: &str, x: i32, y: i32) -> anyhow::Result<()> {
        self.item_proxy(address)
            .await?
            .context_menu(x, y)
            .await
            .map_err(Into::into)
    }

    pub async fn scroll(&self, address: &str, delta: i32, orientation: &str) -> anyhow::Result<()> {
        self.item_proxy(address)
            .await?
            .scroll(delta, orientation)
            .await
            .map_err(Into::into)
    }

    pub async fn about_to_show_menu(
        &self,
        address: String,
        menu_path: String,
        item_id: i32,
    ) -> anyhow::Result<bool> {
        if menu_path.is_empty() {
            return Ok(false);
        }

        self.client
            .about_to_show_menuitem(address, menu_path, item_id)
            .await
            .map_err(Into::into)
    }

    pub async fn activate_menu_item(
        &self,
        address: String,
        menu_path: String,
        item_id: i32,
    ) -> anyhow::Result<()> {
        self.client
            .activate(ActivateRequest::MenuItem {
                address,
                menu_path,
                submenu_id: item_id,
            })
            .await
            .map_err(Into::into)
    }

    async fn item_proxy(&self, address: &str) -> anyhow::Result<StatusNotifierItemProxy<'_>> {
        let (destination, path) = parse_address(address);
        StatusNotifierItemProxy::builder(&self.session)
            .destination(destination.to_string())?
            .path(path)?
            .build()
            .await
            .context("failed to create status notifier item proxy")
    }
}

pub fn snapshot_from_entries(
    entries: &[(String, (StatusNotifierItem, Option<DbusTrayMenu>))],
) -> Snapshot {
    let mut items = entries
        .iter()
        .map(|(address, (item, menu))| map_item(address, item, menu.as_ref()))
        .collect::<Vec<_>>();
    items.sort_by(|left, right| left.address.cmp(&right.address));
    Snapshot { items }
}

pub fn map_item(address: &str, item: &StatusNotifierItem, menu: Option<&DbusTrayMenu>) -> Item {
    Item {
        address: address.to_owned(),
        id: item.id.clone(),
        title: item.title.clone().unwrap_or_default(),
        status: map_status(item.status),
        category: map_category(item.category),
        item_is_menu: item.item_is_menu,
        menu_path: item.menu.clone().unwrap_or_default(),
        icon_theme_path: item.icon_theme_path.clone(),
        icon: map_icon(item.icon_name.as_deref(), item.icon_pixmap.as_deref()),
        overlay_icon: map_icon(
            item.overlay_icon_name.as_deref(),
            item.overlay_icon_pixmap.as_deref(),
        ),
        attention_icon: map_icon(
            item.attention_icon_name.as_deref(),
            item.attention_icon_pixmap.as_deref(),
        ),
        attention_movie_name: item.attention_movie_name.clone(),
        tooltip: map_tooltip(item.tool_tip.as_ref()),
        menu: menu
            .map(|menu| map_menu_items(&menu.submenus))
            .unwrap_or_default(),
    }
}

fn map_icon(name: Option<&str>, pixmaps: Option<&[IconPixmap]>) -> Option<Icon> {
    if let Some(name) = name.filter(|value| !value.trim().is_empty()) {
        return Some(if Path::new(name).is_absolute() {
            Icon::FilePath(name.to_owned())
        } else {
            Icon::Name(name.to_owned())
        });
    }

    select_best_pixmap(pixmaps).map(|pixmap| Icon::Pixmap {
        width: pixmap.width,
        height: pixmap.height,
        pixels: pixmap.pixels.clone(),
    })
}

fn select_best_pixmap(pixmaps: Option<&[IconPixmap]>) -> Option<&IconPixmap> {
    pixmaps?
        .iter()
        .max_by_key(|pixmap| i64::from(pixmap.width.max(0)) * i64::from(pixmap.height.max(0)))
}

fn map_tooltip(tooltip: Option<&Tooltip>) -> Option<super::model::Tooltip> {
    let tooltip = tooltip?;
    Some(super::model::Tooltip {
        title: tooltip.title.clone(),
        description: tooltip.description.clone(),
        icon: map_icon(
            Some(tooltip.icon_name.as_str()).filter(|value| !value.is_empty()),
            Some(tooltip.icon_data.as_slice()),
        ),
    })
}

fn map_menu_items(items: &[DbusMenuItem]) -> Vec<MenuItem> {
    items.iter().map(map_menu_item).collect()
}

fn map_menu_item(item: &DbusMenuItem) -> MenuItem {
    let kind = match item.menu_type {
        MenuType::Separator => MenuItemKind::Separator,
        MenuType::Standard => MenuItemKind::Standard,
    };
    let toggle_type = match item.toggle_type {
        ToggleType::Checkmark => MenuToggleType::Checkmark,
        ToggleType::Radio => MenuToggleType::Radio,
        ToggleType::CannotBeToggled => MenuToggleType::CannotBeToggled,
    };
    let toggle_state = if matches!(toggle_type, MenuToggleType::CannotBeToggled) {
        MenuToggleState::Indeterminate
    } else {
        match item.toggle_state {
            ToggleState::On => MenuToggleState::On,
            ToggleState::Off => MenuToggleState::Off,
            ToggleState::Indeterminate => MenuToggleState::Indeterminate,
        }
    };

    MenuItem {
        id: item.id,
        label: sanitize_label(item.label.as_deref().unwrap_or_default()),
        enabled: matches!(kind, MenuItemKind::Standard) && item.enabled,
        visible: item.visible || matches!(kind, MenuItemKind::Separator),
        kind,
        icon: menu_item_icon(item),
        shortcut: item.shortcut.clone(),
        toggle_type,
        toggle_state,
        children_display: item.children_display.clone(),
        disposition: match item.disposition {
            Disposition::Normal => MenuDisposition::Normal,
            Disposition::Informative => MenuDisposition::Informative,
            Disposition::Warning => MenuDisposition::Warning,
            Disposition::Alert => MenuDisposition::Alert,
        },
        children: map_menu_items(&item.submenu),
    }
}

fn menu_item_icon(item: &DbusMenuItem) -> Option<Icon> {
    if let Some(icon) = map_icon(item.icon_name.as_deref(), None) {
        return Some(icon);
    }

    item.icon_data
        .as_ref()
        .filter(|bytes| !bytes.is_empty())
        .map(|bytes| Icon::EncodedBytes(bytes.clone()))
}

fn sanitize_label(label: &str) -> String {
    label
        .replace("__", "\0")
        .replace('_', "")
        .replace('\0', "_")
}

fn map_category(category: SniCategory) -> Category {
    match category {
        SniCategory::ApplicationStatus => Category::ApplicationStatus,
        SniCategory::Communications => Category::Communications,
        SniCategory::SystemServices => Category::SystemServices,
        SniCategory::Hardware => Category::Hardware,
    }
}

fn map_status(status: SniStatus) -> Status {
    match status {
        SniStatus::Unknown => Status::Unknown,
        SniStatus::Passive => Status::Passive,
        SniStatus::Active => Status::Active,
        SniStatus::NeedsAttention => Status::NeedsAttention,
    }
}

fn event_reason(event: &Event) -> String {
    match event {
        Event::Add(address, _) => format!("add:{address}"),
        Event::Update(address, _) => format!("update:{address}"),
        Event::Remove(address) => format!("remove:{address}"),
    }
}

fn parse_address(address: &str) -> (&str, String) {
    address.split_once('/').map_or(
        (address, String::from("/StatusNotifierItem")),
        |(destination, path)| (destination, format!("/{path}")),
    )
}

#[cfg(test)]
mod tests {
    use system_tray::{
        item::{Category as SniCategory, IconPixmap, Status as SniStatus},
        menu::{Disposition, MenuType, ToggleState, ToggleType},
    };

    use super::*;
    use crate::services::tray::model::Tooltip;

    #[test]
    fn map_item_uses_absolute_icon_name_as_file_path() {
        let mapped = map_item(
            "org.example.App",
            &StatusNotifierItem {
                icon_name: Some("/tmp/example.png".into()),
                ..test_item()
            },
            None,
        );

        assert_eq!(mapped.icon, Some(Icon::FilePath("/tmp/example.png".into())));
    }

    #[test]
    fn map_item_uses_largest_pixmap_when_icon_name_is_missing() {
        let mapped = map_item(
            "org.example.App",
            &StatusNotifierItem {
                icon_name: None,
                icon_pixmap: Some(vec![
                    IconPixmap {
                        width: 16,
                        height: 16,
                        pixels: vec![1; 16 * 16 * 4],
                    },
                    IconPixmap {
                        width: 32,
                        height: 32,
                        pixels: vec![2; 32 * 32 * 4],
                    },
                ]),
                ..test_item()
            },
            None,
        );

        assert_eq!(
            mapped.icon,
            Some(Icon::Pixmap {
                width: 32,
                height: 32,
                pixels: vec![2; 32 * 32 * 4],
            })
        );
    }

    #[test]
    fn map_item_preserves_menu_metadata() {
        let menu = DbusTrayMenu {
            id: 1,
            submenus: vec![
                DbusMenuItem {
                    id: 7,
                    menu_type: MenuType::Standard,
                    label: Some("_Enable".into()),
                    enabled: true,
                    visible: true,
                    icon_name: Some("object-select-symbolic".into()),
                    icon_data: None,
                    shortcut: None,
                    toggle_type: ToggleType::Checkmark,
                    toggle_state: ToggleState::On,
                    children_display: None,
                    disposition: Disposition::Informative,
                    submenu: Vec::new(),
                },
                DbusMenuItem {
                    id: 8,
                    menu_type: MenuType::Separator,
                    ..Default::default()
                },
                DbusMenuItem {
                    id: 9,
                    menu_type: MenuType::Standard,
                    label: Some("_More".into()),
                    enabled: true,
                    visible: true,
                    icon_name: None,
                    icon_data: Some(vec![1, 2, 3, 4]),
                    shortcut: Some(vec![vec!["Control".into(), "M".into()]]),
                    toggle_type: ToggleType::Radio,
                    toggle_state: ToggleState::Off,
                    children_display: Some("submenu".into()),
                    disposition: Disposition::Alert,
                    submenu: vec![DbusMenuItem {
                        id: 10,
                        menu_type: MenuType::Standard,
                        label: Some("Child".into()),
                        enabled: false,
                        visible: true,
                        ..Default::default()
                    }],
                },
            ],
        };

        let mapped = map_item("org.example.App", &test_item(), Some(&menu));

        assert_eq!(
            mapped.tooltip,
            Some(Tooltip {
                title: "Tooltip title".into(),
                description: "Tooltip body".into(),
                icon: Some(Icon::Name("dialog-information-symbolic".into())),
            })
        );
        assert_eq!(
            mapped.menu,
            vec![
                MenuItem {
                    id: 7,
                    label: "Enable".into(),
                    enabled: true,
                    visible: true,
                    kind: MenuItemKind::Standard,
                    icon: Some(Icon::Name("object-select-symbolic".into())),
                    shortcut: None,
                    toggle_type: MenuToggleType::Checkmark,
                    toggle_state: MenuToggleState::On,
                    children_display: None,
                    disposition: MenuDisposition::Informative,
                    children: Vec::new(),
                },
                MenuItem {
                    id: 8,
                    label: String::new(),
                    enabled: false,
                    visible: true,
                    kind: MenuItemKind::Separator,
                    icon: None,
                    shortcut: None,
                    toggle_type: MenuToggleType::CannotBeToggled,
                    toggle_state: MenuToggleState::Indeterminate,
                    children_display: None,
                    disposition: MenuDisposition::Normal,
                    children: Vec::new(),
                },
                MenuItem {
                    id: 9,
                    label: "More".into(),
                    enabled: true,
                    visible: true,
                    kind: MenuItemKind::Standard,
                    icon: Some(Icon::EncodedBytes(vec![1, 2, 3, 4])),
                    shortcut: Some(vec![vec!["Control".into(), "M".into()]]),
                    toggle_type: MenuToggleType::Radio,
                    toggle_state: MenuToggleState::Off,
                    children_display: Some("submenu".into()),
                    disposition: MenuDisposition::Alert,
                    children: vec![MenuItem {
                        id: 10,
                        label: "Child".into(),
                        enabled: false,
                        visible: true,
                        kind: MenuItemKind::Standard,
                        icon: None,
                        shortcut: None,
                        toggle_type: MenuToggleType::CannotBeToggled,
                        toggle_state: MenuToggleState::Indeterminate,
                        children_display: None,
                        disposition: MenuDisposition::Normal,
                        children: Vec::new(),
                    }],
                },
            ]
        );
    }

    #[test]
    fn snapshot_from_entries_is_stably_sorted_by_address() {
        let snapshot = snapshot_from_entries(&[
            ("org.z.Item".into(), (test_item(), None)),
            ("org.a.Item".into(), (test_item(), None)),
        ]);

        assert_eq!(
            snapshot
                .items
                .iter()
                .map(|item| item.address.as_str())
                .collect::<Vec<_>>(),
            vec!["org.a.Item", "org.z.Item"]
        );
    }

    #[test]
    fn labels_preserve_escaped_underscores() {
        assert_eq!(sanitize_label("_Open __Recent"), "Open _Recent");
    }

    #[test]
    fn parse_address_defaults_missing_path() {
        assert_eq!(
            parse_address("org.example.App"),
            ("org.example.App", "/StatusNotifierItem".into())
        );
    }

    #[test]
    fn parse_address_preserves_custom_path() {
        assert_eq!(
            parse_address(":1.42/Custom/Tray"),
            (":1.42", "/Custom/Tray".into())
        );
    }

    fn test_item() -> StatusNotifierItem {
        StatusNotifierItem {
            id: "example".into(),
            category: SniCategory::ApplicationStatus,
            title: Some("Example".into()),
            status: SniStatus::Active,
            window_id: 0,
            icon_theme_path: None,
            icon_name: Some("example-symbolic".into()),
            icon_pixmap: None,
            overlay_icon_name: None,
            overlay_icon_pixmap: None,
            attention_icon_name: None,
            attention_icon_pixmap: None,
            attention_movie_name: None,
            tool_tip: Some(system_tray::item::Tooltip {
                icon_name: "dialog-information-symbolic".into(),
                icon_data: Vec::new(),
                title: "Tooltip title".into(),
                description: "Tooltip body".into(),
            }),
            item_is_menu: false,
            menu: Some("/MenuBar".into()),
        }
    }
}
