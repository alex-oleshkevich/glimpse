...
async fn main() -> Result<(), anyhow::Error> {
    ...
    if dbus::is_running()
        .await
        .map_err(|_| anyhow::anyhow!("glimpse is already running"))?
    {
        tracing::error!("glimpse is already running, exiting.");
        dbus::activate_instance()
            .await
            .map_err(|_| anyhow::anyhow!("failed to activate instance"))?;
        return Ok(());
    }
