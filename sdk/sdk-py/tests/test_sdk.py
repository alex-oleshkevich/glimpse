...
class GlimpseAppletTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_choice_list_serializes_choices(self) -> None:
        node = ChoiceList(id="env", choices=[Choice(id="prod", primary="Production")], active="prod")
        payload = node.to_protocol()
        self.assertEqual(payload["type"], "choice_list")
        self.assertEqual(payload["data"]["choices"][0]["id"], "prod")
    ...
    def test_status_dot_serializes_as_status_protocol_type(self) -> None:
        self.assertEqual(StatusDot().to_protocol()["type"], "status_dot")

    def test_variant_serializes_as_semantic_protocol_value(self) -> None:
        payload = Badge(label="Warning", kind=BadgeKind.WARNING).to_protocol()
        self.assertEqual(payload["data"]["kind"], "warning")
    ...
    async def test_interactive_widgets_register_inline_handlers(self) -> None:
        ...
        children = tree["root"]["data"]["children"]
        generated_ids = [
            children[1]["data"]["id"],
            children[2]["data"]["id"],
            children[3]["data"]["id"],
        ...
        self.assertEqual(children[0]["data"]["id"], "hero-toggle")
        self.assertTrue(generated_ids[0].startswith("__glimpse:click:"))
        self.assertEqual(generated_ids[1], "vpn")
        self.assertEqual(generated_ids[2], "volume")
        self.assertEqual(generated_ids[3], "env")
        ...
        events = [
            ...
            {"id": generated_ids[2], "type": "change", "source": "popover", "value": 0.72},
            {"id": generated_ids[3], "type": "change", "source": "popover", "value": "prod"},
        ]
        for event in events:
            await applet._dispatch_callback(parse_callback_event(event))

        self.assertEqual(applet.state.seen, 5)
        self.assertEqual(
            hits,
            [
                ...
                ("change", generated_ids[2], 0.72),
                ("change", generated_ids[3], "prod"),
            ],
        )
