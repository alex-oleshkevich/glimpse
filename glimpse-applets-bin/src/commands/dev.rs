...
#[derive(ClapArgs)]
pub struct Args {
    /// Project directory (containing Cargo.toml / package.json / go.mod /
    /// main.py). Defaults to the current directory.
    #[arg(value_name = "PATH")]
    path: Option<PathBuf>,
...
fn install_dev_config(name: &str, plan: &Plan) -> Result<PathBuf> {
    ...
    let mut exec_table = toml::map::Map::new();
    exec_table.insert(
        "command".into(),
    ...
    root_table.insert("id".into(), toml::Value::String(name.to_string()));
    root_table.insert("type".into(), toml::Value::String("exec".into()));
    root_table.insert("exec".into(), toml::Value::Table(exec_table));

    let body = toml::to_string(&toml::Value::Table(root_table)).context("serialize dev config")?;
...
fn check_dev_panel_config(applet_name: &str) {
    ...
    let panels = match value.get("panels").and_then(|v| v.as_array()) {
        ...
        None => {
            log(format!(
                "warning: add \"__dev__\" to a panel's left/center/right in {} to see {applet_name} in the bar",
                config_file.display()
            ));
            return;
        }
    ...
    let has_dev = panels.iter().any(|panel| {
        ["left", "center", "right"].iter().any(|section| {
            panel
                ...
                .map(|arr| arr.iter().any(|item| item.as_str() == Some("__dev__")))
                .unwrap_or(false)
        })
    });
    ...
    if !has_dev {
        log(format!(
            "warning: \"__dev__\" is not in any panel's left/center/right in {} — add it to see {applet_name} in the bar",
            config_file.display()
        ));
    }
}
        ...
        move |result: notify_debouncer_full::DebounceEventResult| {
            if let Ok(events) = result {
                let interesting = events.iter().any(|e| {
                    // Ignore metadata-only events (atime, permissions, etc.).
                    // uv reads main.py to parse inline deps, which can flip
                    // atime and generate IN_ATTRIB — without this filter that
                    // would trigger an infinite restart loop.
                    if matches!(
                        e.kind,
