...
pub use imp::{Drive, Entry, Trash, Volume};
...
impl PlacesPopover {
    ...
    pub fn set_overflow(&self, bookmarks: Option<&str>, devices: Option<&str>) {
        let imp = self.imp();
        set_footer_row(&imp.bookmarks_more, bookmarks);
        set_footer_row(&imp.devices_more, devices);
    }
    ...
    pub fn connect_more<F: Fn(&Self, &str) + 'static>(&self, f: F) -> glib::SignalHandlerId {
        self.connect_closure(
            "more",
            false,
